...
def _(data_df):
    data_df.head()
    # Count how many rows have no compound name
    missing_names = data_df["Compound"].isna().sum()
    print(f"Missing compound names: {missing_names}")
...
@app.cell
def _(data_df):
    unmatched_ids = data_df[data_df["Compound"].isna()]["ChEBI ID"].unique()
